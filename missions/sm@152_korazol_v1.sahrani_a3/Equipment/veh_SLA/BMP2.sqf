_veh = _this select 0;

_veh addWeaponcargoglobal ["rhs_weap_rpg26", 1];
_veh addItemCargoGlobal ["rhs_mag_f1", 10];
_veh addItemCargoGlobal ["rhs_mag_rdg2_white", 10];
_veh addItemCargoGlobal ["rhs_VOG25", 10];
_veh addItemCargoGlobal ["rhs_30Rnd_762x39mm_polymer_89",10];
_veh addItemCargoGlobal ["rhs_30Rnd_762x39mm_polymer_tracer",5];
_veh addItemCargoGlobal ["vtn_pg7vl",3];
_veh addItemCargoGlobal ["vtn_f7",1];
_veh addItemCargoGlobal ["mkk_VTN_RPK_40b_TRC",5];
_veh addItemcargoglobal ["ACE_quikclot", 20]; 
_veh addItemcargoglobal ["ACE_elasticBandage", 20]; 
_veh addItemcargoglobal ["ACE_fieldDressing", 20]; 
_veh addItemcargoglobal ["ACE_epinephrine", 20];
_veh addItemcargoglobal ["ACE_packingBandage", 20];
_veh addItemcargoglobal ["ACE_morphine", 20];
_veh addItemcargoglobal ["ACE_tourniquet", 10];
_veh addItemcargoglobal ["ACE_splint", 8];
_veh addItemcargoglobal ["tm_62m", 3];

_veh addItemCargoGlobal ["ace_mag_9m111",1];
_veh addItemCargoGlobal ["rhs_beanie",2];

