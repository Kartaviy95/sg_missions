...
[]spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits < 5) then {["Талибы разбиты. Победа обороны", west] call a3a_fnc_endMission;};
		if (west countside playableUnits < 3) then {["Американцы разбиты. Победа атаки", east] call a3a_fnc_endMission;};
		false
	};
};
};