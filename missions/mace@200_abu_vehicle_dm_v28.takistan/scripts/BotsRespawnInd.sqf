...
_tankTypes = ["rhs_t72ba_tv"]; // Типы танков, которые могут быть созданы
_startMarker = "respawn_east"; // Название маркера начальной позиции
private ["_counterInd"];
_tanksArrayInd = []; // Массив для хранения созданных танков
...
_fnc_MAKEBOTTANK = {
    ...
    // Создаем двух ботов
    _groupBot = createGroup [resistance, true];
    _botGunner = _groupBot createUnit ["I_Soldier_VR_F", _startPos, [], 0, "CAN_COLLIDE"];
    _botDriver = _groupBot createUnit ["I_Soldier_VR_F", _startPos, [], 0, "CAN_COLLIDE"];
...
} forEach _tanksArrayInd;

// Основной цикл
while {true} do {
    _counterInd = count _tanksArrayInd; // Получаем текущее количество танков

    // Создаем танк, если количество танков меньше 6
    if (_counterInd < 6) then {
        [_tankTypes, _startMarker] call _fnc_MAKEBOTTANK;
        _counterInd = _counterInd + 1;
    };
	...
	// Проверяем уничтоженные танки
    {
        if (!alive _x) then {
            ...
            _tanksArrayInd = _tanksArrayInd - [_x];
