_veh = _this select 0;

_veh addItemcargoglobal ["rhs_mag_30Rnd_556x45_Mk262_Stanag", 20];
_veh addItemcargoglobal ["rhs_mag_30Rnd_556x45_M196_Stanag_Tracer_Red", 10];
_veh addItemcargoglobal ["mkk_20Rnd_762x51_m80_Mag", 10];
_veh addItemcargoglobal ["rhs_mag_m67", 10];
_veh addItemcargoglobal ["rhs_mag_an_m8hc", 10];
_veh addWeaponcargoglobal ["rhs_weap_m136", 3];
_veh addWeaponcargoglobal ["rhs_weap_M136_hp", 2];

_veh addBackpackcargoglobal ["fatpack_od", 2];

_veh addItemcargoglobal ["rhs_mag_M441_HE", 15];
_veh addItemcargoglobal ["1Rnd_Smoke_Grenade_shell", 10];
_veh addItemcargoglobal ["rhs_mag_M433_HEDP", 10];

_veh addItemCargoGlobal ["ACE_fieldDressing",10];
_veh addItemCargoGlobal ["ACE_elasticBandage",10];
_veh addItemCargoGlobal ["ACE_packingBandage",10];
_veh addItemCargoGlobal ["ACE_quikclot",10];
_veh addItemCargoGlobal ["ACE_morphine",5];
_veh addItemCargoGlobal ["ACE_epinephrine",5];
_veh additemcargoGlobal ["ACE_tourniquet",5];
_veh addItemcargoglobal ["ToolKit", 1];

_veh addItemCargoGlobal ["rhsusf_100Rnd_762x51_m80a1epr",7];
