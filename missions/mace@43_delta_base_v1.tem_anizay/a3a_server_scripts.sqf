...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 2) then {["Оборона понесла тяжелые потери и сдалась!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Атака понесли серьезные потери и сдались", west] call a3a_fnc_endMission;};
		false
	};
};