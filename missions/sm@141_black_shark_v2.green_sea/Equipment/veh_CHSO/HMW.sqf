_veh = _this select 0;

_veh addBackpackcargoglobal ["rhsusf_falconii", 2];


_veh addItemcargoglobal ["mkk_20rnd_762x51_M80_G3", 5];
_veh addItemcargoglobal ["rhssaf_30rnd_556x45_EPR_G36", 10];
_veh addItemcargoglobal ["rhssaf_30rnd_556x45_Tracers_G36", 5];
_veh addItemcargoglobal ["BWA3_200Rnd_556x45_M995_AP", 3];
_veh addItemcargoglobal ["BWA3_200Rnd_556x45_Tracer", 2];
_veh addItemcargoglobal ["BWA3_DM51A1", 10];
_veh addItemcargoglobal ["BWA3_DM32_Blue", 5];
_veh addItemcargoglobal ["vtn_pg7v", 3];
_veh addItemcargoglobal ["vtn_f7", 3];

_veh addWeaponcargoglobal ["rhs_weap_M136_hp", 1];

_veh addItemcargoglobal ["rhs_mag_M441_HE", 10];
_veh addItemcargoglobal ["1Rnd_SmokeBlue_Grenade_shell", 5];

_veh addItemCargoGlobal ["ACE_fieldDressing",15];
_veh addItemCargoGlobal ["ACE_elasticBandage",15];
_veh addItemCargoGlobal ["ACE_packingBandage",15];
_veh addItemCargoGlobal ["ACE_quikclot",15];
_veh addItemCargoGlobal ["ACE_morphine",10];
_veh addItemCargoGlobal ["ACE_epinephrine",10];
_veh additemcargoGlobal ["ACE_tourniquet",10];
