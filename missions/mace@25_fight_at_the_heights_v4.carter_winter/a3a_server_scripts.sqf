...
[] spawn {
	...
	{
		sleep 10;
		if (INDEPENDENT countside playableUnits < 3) then {["НВФ понесли тяжёлые потери. Победа красной стороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 2) then {["ВС РФ понесли тяжёлые потери. Победа синей стороны!", INDEPENDENT] call a3a_fnc_endMission;};
		false
	};
};