...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 2) then {["Американцы Разбиты! Победа красной стороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Русские понесли тяжёлые потери. Победа синей стороны!", west] call a3a_fnc_endMission;};
		false
	};
};