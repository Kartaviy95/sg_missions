...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 1) then {["Террористы были уничтожены! Победа спецназа", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Спецназ понёс тяжёлые потери и отступил. Победа ФАТХ", independent] call a3a_fnc_endMission;};
		false
	};
};