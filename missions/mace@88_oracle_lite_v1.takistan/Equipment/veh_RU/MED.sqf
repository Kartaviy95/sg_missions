_veh = _this select 0;

_veh addItemCargoGlobal ["ACE_fieldDressing",60];
_veh addItemCargoGlobal ["ACE_elasticBandage",60];
_veh addItemCargoGlobal ["ACE_packingBandage",60];
_veh addItemCargoGlobal ["ACE_quikclot",60];
_veh addItemCargoGlobal ["ACE_morphine",30];
_veh addItemCargoGlobal ["ACE_epinephrine",30];
_veh addItemCargoGlobal ["ACE_personalAidKit",15];
_veh addItemCargoGlobal ["ACE_bloodIV_500",20];
_veh addItemCargoGlobal ["ACE_salineIV_500",20];
_veh addItemCargoGlobal ["ACE_surgicalKit",8];
_veh addItemCargoGlobal ["rhs_30Rnd_545x39_7N10_desert_AK",20];
_veh addItemcargoGlobal ["rhs_30Rnd_545x39_AK_plum_green", 16];
_veh addItemCargoGlobal ["rhs_mag_rgd5",6];
_veh addItemCargoGlobal ["SmokeShellGreen",6];
_veh addItemCargoGlobal ["rhs_weap_rsp30_white",10];
