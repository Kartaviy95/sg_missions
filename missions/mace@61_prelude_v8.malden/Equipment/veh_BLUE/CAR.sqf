_veh = _this select 0;

_veh addWeaponcargoglobal ["rhs_weap_m136_HEDP", 1];
_veh addItemCargoGlobal ["mkk_42Rnd_556x45_mk318_AUG",20];
_veh addItemCargoGlobal ["mkk_42Rnd_556x45_m856a1_AUG",10];
_veh addItemcargoglobal ["rhs_mag_m67",10];
_veh addItemcargoglobal ["rhs_mag_an_m8hc",10];
_veh addItemcargoglobal ["rhsusf_100Rnd_556x45_mixed_soft_pouch_coyote",10];
_veh addItemcargoglobal ["BWA3_10Rnd_762x51_G28_AP",10];
_veh addItemcargoglobal ["ACE_quikclot", 20]; 
_veh addItemcargoglobal ["ACE_elasticBandage", 20]; 
_veh addItemcargoglobal ["ACE_fieldDressing", 20]; 
_veh addItemcargoglobal ["ACE_epinephrine", 20];
_veh addItemcargoglobal ["ACE_packingBandage", 20];
_veh addItemcargoglobal ["ACE_morphine", 10];
_veh addItemcargoglobal ["ACE_tourniquet", 10];
_veh addItemcargoglobal ["ACE_splint", 8];