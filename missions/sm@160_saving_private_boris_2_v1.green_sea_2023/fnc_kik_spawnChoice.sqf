...
antilag_radius = 650; // Радиус, в котором нельзя поставить ещё один взвод или отделение
radius = 30; // Радиус поиска ближайшей дороги
radiusAddition = 8; // Добавочное значение к радиусу, дабы ничего не уничтожалось о текстуры при скрытии террайна
errorWaiting = 3; // время, которое висит маркер ошибки
...
hideTerrainOnSpawn = true; // скрывать объекты ландшафта под телепортируемыми. Рекомендуется не использовать. (false)
...
useCustomRoadsFlag = true; // флаг использования кастомных дорог
customRoadClasses = ["CUP_A2_Road_OA_road_invisible"]; // массив всех классов дополнительных дорог
placedInfoPosition = [4406.34,19976.2,0]; // позиция отрисовки посталенных и не поставленных отделений
defaultMapPos = [5389.87,19520.7,0]; // стандартная позиция появления маркера ошибки на карте
markerForDirection = "suda"; // маркер, в направлении которого будут повёрнуты отделения
...
customRoadNowFlag = false; // фраг проверяется ли сейчас кастомная дорога
...
fnc_kik_refrashLists = {	
	...
	_placedTitle setMarkerTextLocal "Расставленные единицы: ";
...
fnc_kik_endSpawnChoice = {
	...
	if (hideRestrictMarkersAfterChoiceEndFlag) then {
		{ _x setMarkerAlpha 0; } forEach (markersCanPlace + markersCantPlace);
	};
	if (hideSquadMarkersAfterFreezeFlag) then {
		[] spawn {
	...
	if (!(missionNamespace getVariable ["choiceEndedby_" + getPlayerUID player, false])) then {
		player onMapSingleClick {
			...
			if (call fnc_kik_checkClick) then {
				...
				["Клик был вне разрешённой или в запрешённой для расстановки зоне!"] call fnc_kik_error;
			};
		}; 
	} else {
