...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["Атакующие понесли тяжёлые потери", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 1) then {["Боевики уничтожены", west] call a3a_fnc_endMission;};
		false
	};
};
...
[] spawn {
	waitUntil
	{sleep 5; !(alive vip)};
	["Лидер боевиков убит", 2] call a3a_fnc_message;
};