...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 2) then {["Американцы понесли тяжелые потери и отступили!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 2) then {["Советы понесли тяжелые потери и отступили!", west] call a3a_fnc_endMission;};
		false
	};
};