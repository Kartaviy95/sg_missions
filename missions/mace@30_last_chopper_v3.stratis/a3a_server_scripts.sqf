...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits < 3) then {["Русские уничтожены, победа обороны!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits < 1) then {["Немцы уничтожены, победа атаки!", east] call a3a_fnc_endMission;};
		false
	};
};