...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 5) then {["Британская армия понеслиа тяжелые потери. Победа обороны.", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Бундесвер понёс тяжёлые потери. Победа атаки.", west] call a3a_fnc_endMission;};
		false
	};
};