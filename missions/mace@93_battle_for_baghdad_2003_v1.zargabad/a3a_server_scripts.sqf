...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 7) then {["США понесли тяжёлые потери и отступили!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 5) then {["Армия Ирака понесла тяжелые потери и была рассеяна!", west] call a3a_fnc_endMission;};
		false
	};
};