_veh = _this select 0;

_veh addItemCargoGlobal ["ACE_fieldDressing",20];
_veh addItemCargoGlobal ["ACE_elasticBandage",20];
_veh addItemCargoGlobal ["ACE_packingBandage",20];
_veh addItemCargoGlobal ["ACE_quikclot",30];
_veh addItemCargoGlobal ["ACE_morphine",10];
_veh addItemCargoGlobal ["ACE_epinephrine",10];
_veh additemcargoGlobal ["ACE_tourniquet",10];

_veh additemcargoGlobal ["rhs_30Rnd_545x39_7N10_AK",15];
_veh additemcargoGlobal ["rhs_100Rnd_762x54mmR",4];
_veh additemcargoGlobal ["rhs_mag_rgd5",10];
_veh additemcargoGlobal ["rhs_mag_an_m8hc",10];
_veh addWeaponcargoGlobal ["mkk_rhs_weap_rpg18",3];
_veh additemcargoGlobal ["rhsusf_20Rnd_762x51_SR25_m118_special_Mag",6];
_veh addWeaponcargoGlobal ["rhs_weap_rshg2",2];
_veh addWeaponcargoGlobal ["launch_NLAW_F",1];
_veh addWeaponcargoGlobal ["SP_Blowpipe",1];
_veh additemcargoGlobal ["mkk_Blowpipe_missile",1];
_veh addBackpackCargoGlobal ["tf_mr6000l",2];
_veh addItemCargoGlobal ["rhs_100Rnd_762x54mmR",8];
_veh addItemCargoGlobal ["ace_mag_9m133f",2];
_veh addItemCargoGlobal ["ace_mag_9m1331_tdh",2];
