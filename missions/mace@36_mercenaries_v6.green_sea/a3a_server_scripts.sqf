...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 2) then {["BEARовцы уничтожены, победа USEC!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 2) then {["USEC уничтожены, победа BEAR!", east] call a3a_fnc_endMission;};
		false
	};
};