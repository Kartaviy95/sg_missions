...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 3) then {["Проправительственные силы понесли недопустимые потери, победа боевиков!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 4) then {["Боевики уничтожены, победа музыкантов и FACA!", east] call a3a_fnc_endMission;};
		false
	};
};