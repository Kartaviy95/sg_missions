...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 2) then {["Зелёные понесли тяжёлые потери. Победа красной стороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Красные понесли тяжёлые потери. Победа зелёной стороны!", independent] call a3a_fnc_endMission;};
		false
	};
};