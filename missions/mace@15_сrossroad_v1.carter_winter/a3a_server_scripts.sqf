...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 1) then {["Боевики понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 1) then {["ОМОН понес тяжёлые потери и отступил!", independent] call a3a_fnc_endMission;};
		false
	};
};