...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 1) then {["ВС РФ понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 4) then {["ВС США понесла тяжелые потери и отступила!", east] call a3a_fnc_endMission;};
		false
	};
};