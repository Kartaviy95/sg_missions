...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 4) then {["Американцы понесли тяжёлые потери и отступили! Победа обороны", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 1) then {["Русские уничтожены! Победа атаки", west] call a3a_fnc_endMission;};
		false
	};
};