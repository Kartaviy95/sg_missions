...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["ЧСО понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["ЧДКЗ понесли тяжелые потери и отступили!", west] call a3a_fnc_endMission;};
		false
	};
};