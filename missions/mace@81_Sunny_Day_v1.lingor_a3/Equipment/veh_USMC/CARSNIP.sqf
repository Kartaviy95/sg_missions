_veh = _this select 0;

_veh addItemcargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_Stanag",10];
_veh addItemcargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red",5];

_veh addItemcargoGlobal ["rhsusf_mag_10Rnd_STD_50BMG_M33",10];
_veh addItemcargoGlobal ["rhsusf_mag_10Rnd_STD_50BMG_mk211",4];

_veh addItemcargoGlobal ["rhsusf_10Rnd_762x51_m993_Mag",10];
_veh addItemcargoGlobal ["rhsusf_10Rnd_762x51_m62_Mag",4];

_veh addItemcargoGlobal ["rhs_uniform_FROG01_d",3];

_veh addItemcargoGlobal ["rhs_mag_m67",10];
_veh addItemcargoGlobal ["rhs_mag_m18_green",10];
_veh addItemcargoGlobal ["rhs_mag_m18_yellow",10];
_veh addItemcargoGlobal ["rhs_mag_m18_red",10];
_veh addItemcargoGlobal ["rhs_mag_m18_purple",10];

_veh addItemCargoGlobal ["ACE_fieldDressing",20];
_veh addItemCargoGlobal ["ACE_elasticBandage",20];
_veh addItemCargoGlobal ["ACE_packingBandage",20];
_veh addItemCargoGlobal ["ACE_quikclot",20];
_veh addItemCargoGlobal ["ACE_morphine",20];
_veh addItemCargoGlobal ["ACE_epinephrine",20];
_veh additemcargoGlobal ["ACE_tourniquet",20];

_veh addItemCargoGlobal ["ACE_rope6",2];
_veh addItemCargoGlobal ["ACE_rope12",2];

_veh addItemcargoGlobal ["ToolKit",1];

_veh addbackpackcargoglobal ["rhsusf_assault_eagleaiii_coy",2];