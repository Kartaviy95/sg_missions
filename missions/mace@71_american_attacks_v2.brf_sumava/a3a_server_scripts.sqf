...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 6) then {["Армия США понесла тяжёлые потери. Победа ВДВ РФ!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Силы ВДВ РФ были рассеяны. Победа Армии США", west] call a3a_fnc_endMission;};
		false
	};
};