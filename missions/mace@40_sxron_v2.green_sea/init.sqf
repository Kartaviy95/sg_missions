...
{ [] spawn {
    ...
    {
        sleep 10;
        if (west countside playableUnits < 5) then {["Полиция понесла тяжелые потери и отступила!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Бандиты понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
        false
    };
};
};