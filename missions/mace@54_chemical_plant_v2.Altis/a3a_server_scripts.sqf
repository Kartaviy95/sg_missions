...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 1) then {["Солдаты США понесли недопустимые потери...", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Солдаты РФ понесли недопустимые потери...", west] call a3a_fnc_endMission;};
		false
	};
};