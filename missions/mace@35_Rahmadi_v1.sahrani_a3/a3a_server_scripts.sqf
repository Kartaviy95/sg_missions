...
[] spawn {
	...
	{
		sleep 10;
		if (EAST countside playableUnits <4) then {["PMC Avangard понесли тяжелые потери и сдались!", WEST] call a3a_fnc_endMission;};
		if (WEST countside playableUnits < 3) then {["Терористы из Картеля понесли тяжелые потери и отступила!", EAST] call a3a_fnc_endMission;};
		false
	};
};