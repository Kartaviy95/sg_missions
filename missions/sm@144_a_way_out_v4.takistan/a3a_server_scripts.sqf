...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 5) then {["Американцы понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Такистанцы понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
		false
	};
};
