_veh = _this select 0;

_veh addItemCargoGlobal ["rhs_weap_M136_hedp",2];
_veh addItemCargoGlobal ["ACE_tourniquet",20];
_veh addItemCargoGlobal ["ACE_fieldDressing",20];
_veh addItemCargoGlobal ["ACE_elasticBandage",20];
_veh addItemCargoGlobal ["ACE_packingBandage",20];
_veh addItemCargoGlobal ["ACE_quikclot",20];
_veh addItemCargoGlobal ["ACE_morphine",20];
_veh addItemCargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_Stanag",50];
_veh addItemcargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_PMAG_Tracer_Red",25];
_veh addItemcargoGlobal ["rhsusf_200rnd_556x45_mixed_box",10];
_veh addItemcargoGlobal ["rhsusf_20Rnd_762x51_SR25_m118_special_Mag",10];
_veh addItemCargoGlobal ["rhs_mag_m67",20];
_veh addItemCargoGlobal ["rhs_mag_an_m8hc",20];
_veh addItemCargoGlobal ["rhs_mag_M441_HE",20];
_veh addBackpackCargoGlobal ["tfa_bp_pointman_mc",4];