...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 5) then {["ЧСО понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (resistance countside playableUnits < 6) then {["ЧДКЗ понесли тяжелые потери и отступили!", resistance] call a3a_fnc_endMission;};
		false
	};
};