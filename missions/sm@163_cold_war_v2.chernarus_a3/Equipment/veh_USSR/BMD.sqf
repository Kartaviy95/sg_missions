_veh = _this select 0;

_veh addItemCargoGlobal ["rhs_30Rnd_545x39_7N10_AK",25];
_veh addItemCargoGlobal ["rhs_30Rnd_545x39_AK_green",15];

_veh addItemCargoGlobal ["rhs_30Rnd_762x39mm_89",5];
_veh addItemCargoGlobal ["rhs_30Rnd_762x39mm_bakelite_tracer",2];

_veh addItemCargoGlobal ["rhs_mag_rgd5",10];
_veh addItemCargoGlobal ["rhs_mag_rdg2_white",5];

_veh addItemCargoGlobal ["mkk_rhs_weap_rpg18",4];

_veh addItemCargoGlobal ["ACE_fieldDressing",20];
_veh addItemCargoGlobal ["ACE_packingBandage",20];
_veh addItemCargoGlobal ["ACE_elasticBandage",20];
_veh addItemCargoGlobal ["ACE_quikclot",20];
_veh addItemCargoGlobal ["ACE_morphine",20];
_veh addItemCargoGlobal ["ACE_epinephrine",20];

_veh addItemCargoGlobal ["rhs_ssh60",2];

_veh addItemCargoGlobal ["rhs_rd54",2];

_veh addItemCargoGlobal ["ToolKit",1];
