...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["Американские морпехи понесли слишком тяжелые потери. Победа обороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 1) then {["Жители деревни понесли слишком тяжелые потери. Победа атаки!", west] call a3a_fnc_endMission;};
		false
	};
};