...
[] spawn {
	...
	{
		sleep 10;
		if (resistance countside playableUnits < 3) then {airstrike = true;sleep 35;["Моджахеды понесли тяжелые потери и отступили!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 2) then {["Оборона понесла тяжелые потери и проиграла!", west] call a3a_fnc_endMission;};
		false
	};
};
