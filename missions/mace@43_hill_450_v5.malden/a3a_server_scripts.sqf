...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 3) then {["ЧВК "Восток" понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 1) then {["MAF понесли тяжелые потери и отступили!", east] call a3a_fnc_endMission;};
		false
	};
};