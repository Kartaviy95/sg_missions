...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["Спецназ понес тяжелые потери и отступил!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Пираты понесли тяжелые потери и отступили!", west] call a3a_fnc_endMission;};
		false
	};
};