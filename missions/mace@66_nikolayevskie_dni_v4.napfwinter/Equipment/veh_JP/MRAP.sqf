_veh = _this select 0;


_veh addItemCargoGlobal ["rhs_weap_M136_hedp",2];
_veh addItemCargoGlobal ["rhs_mag_maaws_TDM_HEAT",1];
_veh addItemCargoGlobal ["rhs_mag_maaws_HEDP",3];
_veh additemcargoGlobal ["mkk_hlc_30rnd_556x45_SOST_HK33",16];
_veh additemcargoGlobal ["rhs_mag_m67",8];
_veh additemcargoGlobal ["rhs_mag_an_m8hc",8];
_veh additemcargoGlobal ["ACE_elasticBandage",20];
_veh additemcargoGlobal ["ACE_quikclot",20];
_veh additemcargoGlobal ["ACE_epinephrine",8];

_veh addBackpackCargoGlobal ["tf_rt1523g_sage",1];
_veh addBackpackCargoGlobal ["BWA3_TacticalPack_Multi",1];