...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["Оборона понесла тяжелые потери и сдалась!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["ЧВК понесли серьезные потери и сдались", west] call a3a_fnc_endMission;};
		false
	};
};