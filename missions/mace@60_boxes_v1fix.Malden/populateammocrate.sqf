...
if (_ammoOnly || _side == WEST || sideweapons>1) then
{
	...
	if (launchers>2) then
	{
		...
		if (!_ammoOnly) then
		{
		_vec addWeaponCargo ["rhs_weap_fim92", 20];
		_vec addWeaponCargo ["rhs_weap_fgm148",20];
		};
			_vec addMagazineCargo ["rhs_fgm148_magazine_AT",20];
			_vec addMagazineCargo ["rhs_fim92_mag",20];
	...
	if (launchers>1) then
	{
		...
		if (!_ammoOnly) then
		{
			_vec addWeaponCargo ["rhs_weap_M136",40];
			_vec addMagazineCargo ["rhs_ammo_M136_rocket",40];
		};
	};
	...
	if (explosives>4) then
	{
		...
		if (!_ammoOnly) then
		{
			_vec addWeaponCargo ["rhs_weap_M320", 20];
			if (futureweapons>0) then
			{
				_vec addWeaponCargo ["rhs_weap_M320", 20];
			};
		};
		_vec addMagazineCargo ["rhs_mag_M441_HE", 80];
		
		_vec addMagazineCargo ["rhs_mag_M441_HE", 40]; //EP1
	...
	if (!_ammoOnly) then
	{
		_vec AddWeaponCargo ["RH_deagle", 20];
	...
	_vec addmagazinecargo ["RH_7Rnd_50_AE", 60];
	...
	if (weaponTypes>0) then
	{
		if (!_ammoOnly) then
		{
			...
			_vec AddWeaponCargo ["hlc_smg_mp5a4", 20];
		...
		_vec addmagazinecargo ["hlc_30Rnd_9x19_B_MP5", 80];
	...
	if (weaponTypes>2 && caliber>2) then
	{
		...
		if (!_ammoOnly) then
		{
			...
			_vec AddWeaponCargo ["rhs_weap_m4a1_grip", 20];
		...
		_vec addmagazinecargo ["rhs_mag_30Rnd_556x45_M855A1_Stanag", 80];
...
if (_ammoOnly || _side == EAST || sideweapons>1) then
{
	...
	if (weaponTypes>2 && caliber>2) then
	{
		...
		if (!_ammoOnly) then
		{
			...
			_vec addWeaponCargo ["rhs_weap_ak74m_2mag_dtk", 20];
		...
		if (explosives>0) then
		{
			_vec addMagazineCargo ["rhs_VOG25P", 60];
		};
		if (lessThanLethal>0) then
		{
		...
		_vec addmagazinecargo ["rhs_30Rnd_545x39_AK", 80];
		_vec addMagazineCargo ["rhs_VG40OP_white", 60];
...
if (!_ammoOnly) then
{
	_vec AddWeaponCargo ["FlareLauncher", 20];
};
_vec addMagazineCargo ["FlareLauncherMag", 60];
...
_vec addWeaponCargo ["Rangefinder",20];
_vec addWeaponCargo ["Binocular_Vector",20];
...
_vec addMagazineCargo ["ACE_Bandage",50];
_vec addMagazineCargo ["ACE_Epinephrine",50];
_vec addMagazineCargo ["ACE_Morphine",50];
...
{
	// satchel
	_vec addMagazineCargo ["DemoCharge_Remote_Mag",60];
};
