...
//sndsrc = [vert1, vert3, vert10, vert11];
[] spawn { 
	waitUntil { sleep 3; a3a_var_started};
gener = false;
publicvariable "gener";
endSong = false;
publicvariable "endSong";
...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 4) then {["Атака понесла тяжелые потери. Победа обороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Оборона понесла тяжелые потери. Победа атаки!", west] call a3a_fnc_endMission;};
		false
	};
};
