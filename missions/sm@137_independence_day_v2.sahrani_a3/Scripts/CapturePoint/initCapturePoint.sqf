...
[
	...
	"Баганго", // название зоны, которое будет отображено в нотификациях, текст перед названием зоны "Начат захват ..."
	"Баганго был захвачен!", // текст, который будет показан в нотификациях после захвата
...
[
	...
	west, // сторона которая владеет зоной на старте, может быть west/east/independent
	240, // время захвата в секундах
	"фабрики", // название зоны, которое будет отображено в нотификациях, текст перед названием зоны "Начат захват ..."
	"Фабрика была захвачена!", // текст, который будет показан в нотификациях после захвата
...
_zones set ["z1", east]; // [название маркера, какая сторона должна владеть зоной для победы]
_zones set ["z2", east]; // [название маркера, какая сторона должна владеть зоной для победы]
...
_winningMessage = "ВДВ РФ захватила плацдарм!"; // сообщение, которое показывается при захвате требуемого количества зон
