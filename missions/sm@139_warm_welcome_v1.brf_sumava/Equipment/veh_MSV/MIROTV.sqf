_veh = _this select 0;

_veh addItemCargoGlobal ["ACE_tourniquet",16];
_veh addItemCargoGlobal ["ACE_fieldDressing",30];
_veh addItemCargoGlobal ["ACE_elasticBandage",30];
_veh addItemCargoGlobal ["ACE_packingBandage",30];
_veh addItemCargoGlobal ["ACE_quikclot",30];
_veh addItemCargoGlobal ["ACE_morphine",20];
_veh addItemCargoGlobal ["ACE_epinephrine",20];
_veh addItemCargoGlobal ["rhs_30Rnd_545x39_7N10_AK",40];
_veh addItemcargoGlobal ["rhs_100Rnd_762x54mmR_7N13", 20];
_veh addItemCargoGlobal ["rhs_rpg7_PG7VL_mag",4];
_veh addItemCargoGlobal ["rhs_rpg7_OG7V_mag",4];
_veh addItemCargoGlobal ["rhs_mag_rgd5",30];
_veh addItemCargoGlobal ["rhs_mag_rdg2_white",20];
_veh addItemCargoGlobal ["rhs_weap_rpg26",2];
_veh addItemCargoGlobal ["rhs_weap_rshg2",2];
_veh addItemCargoGlobal ["rhs_30Rnd_545x39_AK_plum_green",20];
_veh addItemCargoGlobal ["rhs_VOG25",40];
_veh addItemCargoGlobal ["ACE_EntrenchingTool",5];
_veh addItemCargoGlobal ["rhs_mine_pmn2_mag",5];
_veh addItemCargoGlobal ["ACE_FlareTripMine_Mag",10];

_veh addBackpackCargoGlobal ["rhs_sidor", 1];
