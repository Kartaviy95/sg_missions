...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 5) then {["ССО РФ понесли тяжёлые потери и отошли, победа американцев!", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 4) then {["Американцы уничтожены, победа ССО РФ!", east] call a3a_fnc_endMission;};
		false
	};
};