...
if ({(side _x == east) && (isPlayer _x)} count playableUnits < 4) then {
	...
	if ({(side _x == east) && (isPlayer _x)} count playableUnits < 6) then {
		deleteVehicle panzer_3;
	};
};

if ({(side _x == west) && (isPlayer _x)} count playableUnits < 4) then {
	deleteVehicle tank_2;
	deleteVehicle tank_3;
} else {
	if ({(side _x == west) && (isPlayer _x)} count playableUnits < 6) then {
		deleteVehicle tank_3;
	};
};
