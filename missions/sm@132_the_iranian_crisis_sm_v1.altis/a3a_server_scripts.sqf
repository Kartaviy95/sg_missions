...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 5) then {["Армия США понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 7) then {["Армия Ирана понесла тяжелые потери и отступила!", west] call a3a_fnc_endMission;};
		false
	};
};