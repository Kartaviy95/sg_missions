...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 3) then {["Зельоние понесли тяжёлые потери. Победа красной стороны!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Красные понесли тяжёлые потери. Победа синей стороны!", independent] call a3a_fnc_endMission;};
		false
	};
};