...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["Тени понесли тяжелые потери и отступили", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Красный спецназ понес тяжелые потери и сдался!", west] call a3a_fnc_endMission;};
		false
	};
};