...
[] spawn {
	...
	{
		sleep 10;
		if (east countside playableUnits <= 2) then {["Российские военные понесли тяжёлые потери и отошли, победа ЧСО", west] call a3a_fnc_endMission;};
		if (west countside playableUnits <= 4) then {["Нацгвардия Чернаруси понесла тяжёлые потери и отступила, победа ВС РФ", east] call a3a_fnc_endMission;};
		false
	};
};