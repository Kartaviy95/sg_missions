...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 4) then {["Спецназ понёс тяжелые потери и отступил!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Террористы понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
		false
	};
};
[] spawn {
