...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 5) then {["Боевики понесли тяжелые потери и отступили!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Десантники понесли тяжелые потери и сдались!", independent] call a3a_fnc_endMission;};
		false
	};
};