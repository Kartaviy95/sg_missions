_veh = _this select 0;

_veh addItemCargoGlobal ["rhs_weap_M136_hedp",1];
_veh addItemCargoGlobal ["rhs_weap_M136",1];

_veh addItemCargoGlobal ["ACE_fieldDressing",12];
_veh addItemCargoGlobal ["ACE_elasticBandage",12];
_veh addItemCargoGlobal ["ACE_packingBandage",12];
_veh addItemCargoGlobal ["ACE_quikclot",12];
_veh addItemCargoGlobal ["ACE_morphine",12];
_veh addItemCargoGlobal ["ACE_epinephrine",12];
_veh addItemCargoGlobal ["ACE_tourniquet",12];
_veh addItemCargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_PMAG",16];
_veh addItemcargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_PMAG_Tracer_Red",8];
_veh addItemCargoGlobal ["rhs_mag_20Rnd_SCAR_762x51_m80a1_epr_bk",16];
_veh addItemcargoGlobal ["rhs_mag_20Rnd_SCAR_762x51_m62_tracer",8];
_veh addItemcargoGlobal ["rhs_mag_M441_HE",16];
_veh addItemcargoGlobal ["rhs_mag_M433_HEDP",12];
_veh addItemcargoGlobal ["rhsusf_200Rnd_556x45_soft_pouch",6];
_veh addItemcargoGlobal ["rhsusf_20Rnd_762x51_SR25_m118_special_Mag",8];
_veh addItemCargoGlobal ["rhs_mag_m67",14];
_veh addItemCargoGlobal ["rhs_mag_an_m8hc",14];
_veh addItemCargoGlobal ["rhsusf_mich_bare_norotos",2];
_veh addItemCargoGlobal ["ToolKit",1];
_veh addItemCargoGlobal ["ace_mag_TOW2b",2];			
_veh addItemCargoGlobal ["ace_mag_TOW2BB_aero",2];			
_veh addItemCargoGlobal ["ace_mag_TOW2BB",2];			

_veh addItemCargoGlobal ["ACE_rope6",2];
_veh addItemCargoGlobal ["ACE_rope12",2];