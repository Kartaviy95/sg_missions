...
{ [] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 6) then {["ВС США понесли тяжелые потери. Победа атаки!", EAST] call a3a_fnc_endMission;};
		if (east countside playableUnits < 8) then {["ВС РФ понесли тяжелые потери. Победа обороны!", WEST] call a3a_fnc_endMission;};
		false
	};
};
};