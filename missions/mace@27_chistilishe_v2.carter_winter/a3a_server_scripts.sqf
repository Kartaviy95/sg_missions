...
[] spawn { 
	...
	while {isNil "srv_triggerFinished"} do { 
		if (((diag_tickTime - _srv_gameStart) > _missionEndTime)) then { 
			...
			["Время истекло. Колона приехала!", west] call a3a_fnc_endMission; // Сообщение о победе, выигравшая сторона
		}; 
		sleep 1;
	};
};
...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 2) then {["УНА УНСО понесли тяжелые потери и отступили!!", east] call a3a_fnc_endMission;};
