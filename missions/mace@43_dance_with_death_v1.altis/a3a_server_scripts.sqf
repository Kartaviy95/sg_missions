...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits == 0) then {["Все синие уничтожены!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits == 0) then {["Все красные уничтожены!", west] call a3a_fnc_endMission;};
		false
	};
};
