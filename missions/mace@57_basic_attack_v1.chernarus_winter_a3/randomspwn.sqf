if (isServer) then {
_rnd = selectrandom ["0","1","2","3","4","5"];


if (_rnd == "0") then {};
if (_rnd == "1") then {
trigan setpos getmarkerpos "postrig2";
"trigmark" setmarkerpos getpos trigan;
"zone" setmarkerpos getpos trigan;
hum1 setpos getmarkerpos "poshum11";
hum2 setpos getmarkerpos "poshum21";
hum3 setpos getmarkerpos "poshum31";
hum4 setpos getmarkerpos "poshum41";
hum5 setpos getmarkerpos "poshum51";
hum6 setpos getmarkerpos "poshum61";	
	
};

if (_rnd == "2") then {
trigan setpos getmarkerpos "postrig2_1";
"trigmark" setmarkerpos getpos trigan;
"zone" setmarkerpos getpos trigan;
hum1 setpos getmarkerpos "poshum11_1";
hum2 setpos getmarkerpos "poshum21_1";
hum3 setpos getmarkerpos "poshum31_1";
hum4 setpos getmarkerpos "poshum41_1";
hum5 setpos getmarkerpos "poshum51_1";
hum6 setpos getmarkerpos "poshum61_1";	
	
};
if (_rnd == "3") then {
trigan setpos getmarkerpos "postrig2_2";
"trigmark" setmarkerpos getpos trigan;
"zone" setmarkerpos getpos trigan;
hum1 setpos getmarkerpos "poshum11_2";
hum2 setpos getmarkerpos "poshum21_2";
hum3 setpos getmarkerpos "poshum31_2";
hum4 setpos getmarkerpos "poshum41_2";
hum5 setpos getmarkerpos "poshum51_2";
hum6 setpos getmarkerpos "poshum61_2";	
	
};

if (_rnd == "4") then {
trigan setpos getmarkerpos "postrig2_3";
"trigmark" setmarkerpos getpos trigan;
"zone" setmarkerpos getpos trigan;
hum1 setpos getmarkerpos "poshum11_3";
hum2 setpos getmarkerpos "poshum21_3";
hum3 setpos getmarkerpos "poshum31_3";
hum4 setpos getmarkerpos "poshum41_3";
hum5 setpos getmarkerpos "poshum51_3";
hum6 setpos getmarkerpos "poshum61_3";	
	
};
if (_rnd == "5") then {
trigan setpos getmarkerpos "postrig2_4";
"trigmark" setmarkerpos getpos trigan;
"zone" setmarkerpos getpos trigan;
hum1 setpos getmarkerpos "poshum11_4";
hum2 setpos getmarkerpos "poshum21_4";
hum3 setpos getmarkerpos "poshum31_4";
hum4 setpos getmarkerpos "poshum41_4";
hum5 setpos getmarkerpos "poshum51_4";
hum6 setpos getmarkerpos "poshum61_4";	
	
};


_rnd1 = selectrandom ["0","1","2","3"];


if (_rnd1 == "0") then {};
if (_rnd1 == "1") then {

uaz setpos getmarkerpos "uaz_1";
btr1 setpos getmarkerpos "btr1_1";
btr2 setpos getmarkerpos "btr2_1";
kamaz setpos getmarkerpos "kamaz_1";
	
	
};

if (_rnd1 == "2") then {

uaz setpos getmarkerpos "uaz_2";
btr1 setpos getmarkerpos "btr1_2";
btr2 setpos getmarkerpos "btr2_2";
kamaz setpos getmarkerpos "kamaz_2";
	
};
if (_rnd1 == "3") then {

uaz setpos getmarkerpos "uaz_3";
btr1 setpos getmarkerpos "btr1_3";
btr2 setpos getmarkerpos "btr2_3";
kamaz setpos getmarkerpos "kamaz_3";
	
};

};