...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 4) then {["Солдаты Израиля понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 5) then {["Террористы понесли тяжелые потери и отступили!", west] call a3a_fnc_endMission;};
		false
	};
};