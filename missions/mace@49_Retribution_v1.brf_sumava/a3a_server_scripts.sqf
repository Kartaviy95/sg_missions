...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 5) then {["Терористы понесли тяжелые потери и отступили!", WEST] call a3a_fnc_endMission;};
		if (WEST countside playableUnits < 4) then {["ДПСУ понесли тяжелые потери и сдались!", independent] call a3a_fnc_endMission;};
		false
	};
};