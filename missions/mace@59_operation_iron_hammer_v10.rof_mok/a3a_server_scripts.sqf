...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 4) then {["Оборона понесла тяжелые потери! Победа атаки!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 5) then {["Атака понесла тяжелые потери! Победа обороны!", independent] call a3a_fnc_endMission;};
		false
	};
};