...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 2) then {["Армия США понесла тяжелые потери и отступила!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 1) then {["Армия РФ понесла тяжелые потери и проиграла!", west] call a3a_fnc_endMission;};
		false
	};
};