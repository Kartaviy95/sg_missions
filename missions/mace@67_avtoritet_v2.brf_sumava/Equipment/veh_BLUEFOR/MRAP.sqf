// это пример того как должен выглядеть файл со снаряжением для техники
_veh addItemCargoGlobal ["rhs_mag_m67", 10];
_veh addItemCargoGlobal ["SmokeShell", 10];
_veh addItemCargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_Stanag", 15];
_veh addItemCargoGlobal ["rhs_mag_30Rnd_556x45_M855_PMAG_Tan_Tracer_Red", 7];
_veh addItemCargoGlobal ["rhsusf_200rnd_556x45_M855_box", 3];
_veh addItemCargoGlobal ["rhsusf_200rnd_556x45_M855_mixed_box", 3];
_veh addItemCargoGlobal ["rhsusf_20Rnd_762x51_m118_special_Mag", 10];
_veh addItemCargoGlobal ["ACE_tourniquet", 8];
_veh addItemCargoGlobal ["ACE_fieldDressing", 10];
_veh addItemCargoGlobal ["ACE_elasticBandage", 10];
_veh addItemCargoGlobal ["ACE_packingBandage", 10];
_veh addItemCargoGlobal ["ACE_quikclot", 10];
_veh addItemCargoGlobal ["ACE_morphine", 8];
_veh addItemCargoGlobal ["ACE_epinephrine", 8];
_veh addItemCargoGlobal ["ace_mag_100rnd_127x99_mag_Tracer_Red", 10];
_veh addItemCargoGlobal ["ACE_splint", 10];
_veh addItemCargoGlobal ["ace_mag_TOW2A",4];
_veh addItemCargoGlobal ["ace_mag_TOW2BB_aero",2];
_veh addItemCargoGlobal ["ace_mag_TOW2b",1];
// рюкзаки добавляются отдельной командой
_veh addBackpackCargoGlobal ["rhs_tortila_grey", 1];
_veh addBackpackCargoGlobal ["mkk_Bag_Felin_45L_Radio", 1];