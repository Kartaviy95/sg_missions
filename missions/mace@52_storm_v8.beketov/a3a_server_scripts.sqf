...
[] spawn {
	...
	{
		sleep 10;
		if (WEST countside playableUnits < 3) then {["Оборона понесла тяжелые потери! Победа атаки!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Атака понесла тяжелые потери! Победа обороны!", WEST] call a3a_fnc_endMission;};
		false
	};
};