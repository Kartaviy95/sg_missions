...
[] spawn {
	...
	{
		sleep 10;
		if (INDEPENDENT countside playableUnits <4) then {["ВДВ РФ понесли тяжелые потери и сдались!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 3) then {["Ополченцы понесли тяжелые потери и отступила!", INDEPENDENT] call a3a_fnc_endMission;};
		false
	};
};