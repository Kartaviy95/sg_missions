_this addItemcargoglobal ["ToolKit", 1];

_this addItemcargoGlobal ["nmg_30Rnd_545x39_7N10_AK12",10];
_this addItemcargoGlobal ["nmg_30Rnd_545x39_7T3M_AK12",5];
_this addItemcargoGlobal ["rhs_mag_rgd5",4];
_this addItemcargoGlobal ["rhs_mag_rdg2_white",4];

_this addItemCargoGlobal ["ACE_fieldDressing",20];
_this addItemCargoGlobal ["ACE_elasticBandage",20];
_this addItemCargoGlobal ["ACE_packingBandage",20];
_this addItemCargoGlobal ["ACE_quikclot",20];
_this addItemCargoGlobal ["ACE_morphine",10];
_this addItemCargoGlobal ["ACE_epinephrine",10];
_this additemcargoGlobal ["ACE_tourniquet",10];
_this additemcargoGlobal ["ACE_splint",10];

_this addItemCargoGlobal ["ACE_rope6",1];
_this addItemCargoGlobal ["ACE_rope12",1];

_this addItemCargoGlobal ["rhsusf_ach_bare",3];
_this addBackpackcargoglobal ["B_Kitbag_sgg", 2];

//БК к t80
_this addItemCargoGlobal ["mkk_csw_cent_mag_m735",15];
_this addItemCargoGlobal ["mkk_csw_cent_mag_m456",10];
_this addItemCargoGlobal ["mkk_csw_cent_mag_m393a2",12];
_this addItemCargoGlobal ["BWA3_120Rnd_762x51",10];