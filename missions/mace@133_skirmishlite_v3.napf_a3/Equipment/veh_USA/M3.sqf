_veh = _this select 0;

_veh addItemCargoGlobal ["rhs_weap_M136",1];
_veh addItemCargoGlobal ["rhs_weap_M136_hp",1];

_veh addItemcargoGlobal ["rhs_mag_maaws_TDM_HEAT",2];
_veh addItemcargoGlobal ["rhs_mag_maaws_HE",2];

_veh addItemCargoGlobal ["ACE_fieldDressing",10];
_veh addItemCargoGlobal ["ACE_elasticBandage",10];
_veh addItemCargoGlobal ["ACE_packingBandage",10];
_veh addItemCargoGlobal ["ACE_quikclot",10];
_veh addItemCargoGlobal ["ACE_morphine",10];
_veh addItemCargoGlobal ["ACE_epinephrine",10];
_veh addItemCargoGlobal ["ACE_tourniquet",10];
_veh addItemCargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_PMAG",12];
_veh addItemcargoGlobal ["rhs_mag_30Rnd_556x45_M855A1_PMAG_Tracer_Red",8];
_veh addItemCargoGlobal ["rhs_mag_20Rnd_SCAR_762x51_m80a1_epr_bk",12];
_veh addItemcargoGlobal ["rhs_mag_20Rnd_SCAR_762x51_m62_tracer",8];
_veh addItemcargoGlobal ["rhs_mag_M441_HE",16];
_veh addItemcargoGlobal ["rhsusf_200Rnd_556x45_soft_pouch",4];
_veh addItemCargoGlobal ["rhs_mag_m67",10];
_veh addItemCargoGlobal ["rhs_mag_an_m8hc",8];
_veh addItemCargoGlobal ["rhsusf_mich_bare_norotos",2];
_veh addItemCargoGlobal ["ToolKit",1];

_veh addItemCargoGlobal ["ACE_rope6",2];
_veh addItemCargoGlobal ["ACE_rope12",2];