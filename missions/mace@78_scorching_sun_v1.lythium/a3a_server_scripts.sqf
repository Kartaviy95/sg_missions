...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 5) then {["ЦАХАЛ понес тяжелые потери и отступил!Победа обороны", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Террористы понесли тяжелые потери и сдались!", west] call a3a_fnc_endMission;};
		false
	};
};