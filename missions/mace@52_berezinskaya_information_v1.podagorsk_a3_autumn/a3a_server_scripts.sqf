...
[] spawn {
	...
	{
		sleep 10;
		if (resistance countside playableUnits < 3) then {["Боевики понесли тяжелые потери, заложники спасены", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["МСВ РБ разбиты, победа обороны!", resistance] call a3a_fnc_endMission;};
		false
	};
};