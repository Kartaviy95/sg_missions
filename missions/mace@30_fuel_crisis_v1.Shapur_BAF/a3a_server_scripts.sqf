...
_winningMessage = "Время истекло. АОШ сохранила хотя бы один УРАЛ!"; // Сообщение которое показывается по окончанию миссии

[_missionTime, _winningSide, _winningMessage] spawn ATRIUM_fnc_timeout;

// Параметры для завершения миссии по потерям
_bluforThreshold = 1; // минимальное количество живых синих юнитов
_bluforMessage = "ААШ понесла тяжелые потери!"; // сообщение если проиграли синие
_bluforEnemySide = opfor; // кто выигрывает в случае больших потерь синих

_opforThreshold = 1; // минимальное количество живых красных юнитов
_opforMessage = "АОШ понесли тяжелые потери!"; // сообщение если проиграли красные
