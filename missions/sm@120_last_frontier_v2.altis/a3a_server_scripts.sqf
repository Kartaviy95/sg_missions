...
[] spawn {
	...
	{
		sleep 10;
		if (independent countside playableUnits < 3) then {["Боевики были уничтожены. Победа Сирийской армии!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 6) then {["Сирийская армия понесла тяжёлые потери. Победа боевиков!", independent] call a3a_fnc_endMission;};
		false
	};
};