...
[] spawn {
	...
	{
		sleep 10;
		if (west countside playableUnits < 3) then {["ИЖИЛ понесли тяжелые потери и отступили!", east] call a3a_fnc_endMission;};
		if (east countside playableUnits < 4) then {["Оборона понесла тяжелые потери и сдалась!", west] call a3a_fnc_endMission;};
		false
	};
};