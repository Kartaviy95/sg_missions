...
_winningMessage = "Время вышло. ЧДКЗ удержали терминал!"; // Сообщение которое показывается по окончанию миссии
...
_opforMessage = "Победа Атаки, Сепаратисты ЧДКЗ понесли серьёзные потери!"; // сообщение если проиграли красные
_opforEnemySide = independent; // кто выигрывает в случае больших потерь красных

_independentThreshold = 6; // минимальное количество живых зеленых юнитов
_independentMessage = "Победа Обороны, Киборги ЧСО понесли сокрушительные потери!"; // сообщение если проиграли зеленые
_independentEnemySide = opfor; // кто выигрывает в случае больших потерь синих
