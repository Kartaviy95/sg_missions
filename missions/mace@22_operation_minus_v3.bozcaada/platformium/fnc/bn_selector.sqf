bn_selector_groups = [];
bn_defaults_used = [];
bn_selector_messages = ["ОБЯЗАТЕЛЬНО выберите место старта, доступно 5 вариантов"];
...
BN_SELECTOR_GROUP_PARAMS = ["_title","_max_selected_items","_options","_allow_empty"];
...
bn_selector_create_group = { //[title, maxSelected, allowEmpty]
	params ["_title",["_max_selected_items",1],["_allow_empty",false]];
	bn_selector_groups pushBack [_title, _max_selected_items, [],_allow_empty];
...
bn_selector_paint = { // [start_coords, step_y, "actions after commit"]
	...
	{
		private _marker = ["bn_selector_message_" + (str (count bn_selector_markers)),_pos_x,_pos_y,0.5,"mil_dot_noShadow","ColorRed",_x] call bn_selector_marker_icon;
	...
	} forEach bn_selector_messages;
	{
		...
		private _marker = ["bn_selector_group_" + (str _group_index),_pos_x,_pos_y,0.5,"mil_dot_noShadow","ColorRed",_title] call bn_selector_marker_icon;
		_pos_y = _pos_y + _STEP_Y;
		_pos_x = _pos_x + _STEP_X; // indent
		{
			...
			private _marker = ["bn_selector_item_" + (str _group_index) + "_" + (str _forEachIndex),_marker_pos select 0,_marker_pos select 1,BN_SELECTED_MARKER_SIZE,BN_SELECTABLE_MARKER,"ColorRed",_item_title] call bn_selector_marker_icon;
			if (_make_step) then {
				_pos_y = _pos_y + _STEP_Y;
			};
		} forEach _options;
